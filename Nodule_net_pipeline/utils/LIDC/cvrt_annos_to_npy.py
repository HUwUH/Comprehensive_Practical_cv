import sys
sys.path.append(r'E:\work_files\praticalTraining_cv\NoduleNet')
...
def annotation2masks(annos_dir, save_dir):
    ...
    for f in tqdm(files, total=len(files)):
        try:
            ...
            #     f.write(str(masks))
            np.save(os.path.join(save_dir, '%s' % (seriesuid)), 
    ...
    for pid in tqdm(pids, total=len(pids)):
        ...
        for n in range(num.max(), 0, -1):
            ...
            for i, index in enumerate(np.where(num >= n)[0]):
                ...
                m = np.logical_or.reduce(same_nodules)  #lby: 我感觉这样可能会提高召回率但一定程度减少精确度
                mask[m] = i + 1
...
if __name__ == '__main__':
    ...
    print(f"annos_dir:{annos_dir}")
    print(f"img_dir:{img_dir}")
    print(f"ctr_arr_save_dir:{ctr_arr_save_dir}")
