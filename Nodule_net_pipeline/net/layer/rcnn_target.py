import copy
from Nodule_net_pipeline.net.layer.rcnn_nms import rcnn_encode
from Nodule_net_pipeline.net.layer.mask_target import make_one_mask_target
import time
import random
import numpy as np
import torch
...
try:
    ...
    from Nodule_net_pipeline.utils.util import py_nms as torch_nms
    from Nodule_net_pipeline.utils.util import py_box_overlap as torch_overlap
...
def make_one_rcnn_target(cfg, input, proposal, truth_box, truth_label):
    ...
    # 添加类型转换
    sampled_target = np.asarray(sampled_target, dtype=np.float32)
    sampled_label = np.asarray(sampled_label, dtype=np.int64)
    sampled_proposal = np.asarray(sampled_proposal, dtype=np.float32)
