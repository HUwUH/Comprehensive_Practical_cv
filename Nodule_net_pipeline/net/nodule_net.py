...
from Nodule_net_pipeline.net.layer import *

from Nodule_net_pipeline.config import net_config as config
import copy
from torch.nn.parallel.data_parallel import data_parallel
import time
import torch.nn.functional as F
from Nodule_net_pipeline.utils.util import center_box_to_coord_box, ext2factor, clip_boxes
...
class NoduleNet(nn.Module):
    ...
    def forward(self, inputs, truth_boxes, truth_labels, truth_masks, masks, split_combiner=None, nzhw=None):
        features, feat_4 = data_parallel(self.feature_net, (inputs)); print('features[-1] shape:', features[-1].shape)
        fs = features[-1]

        self.rpn_logits_flat, self.rpn_deltas_flat = data_parallel(self.rpn, fs)
        # print(self.rpn_logits_flat)
        # print(self.rpn_deltas_flat)
        ...
        if self.use_rcnn:
            ...
            if self.rpn_proposals.shape[0] == 0:
                print("[LBY_DEBUG] No valid RPN proposals found for this image.")
            ...
            if self.use_mask and len(self.detections):
                ...
                if len(self.crop_boxes) > MAX_BOXES:
                    self.crop_boxes = self.crop_boxes[:MAX_BOXES]
    ...
    def forward2(self, inputs, bboxes):# 似乎没有用到过，可能是人家废弃的代码
        assert False, "这里我感觉不会被运行到"
