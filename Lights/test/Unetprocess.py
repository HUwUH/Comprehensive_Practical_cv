...
def dcm_to_jpg_array(dcm_path, window_center=-600, window_width=1600):
    ...
    Args:
        ...
        global_index = save_masks_for_unet_optimized(dicom_folder, unet_train_folder, global_index)_center: 窗位
        window_width: 窗宽
    Returns:
        ...
        for n in nods:
            ...
            if not found:
                grouped.append({
                    ...
                    "filename": n['filename'],
                    "centers": [n['center']],
                    "edge_points_list": [n['edge_points']],                    "malignancies": [n['malignancy']]
                })
        for group in grouped:
            avg_center = [
...
def create_mask_from_points(edge_points, image_size=(512, 512)):
    ...
    """
    # 创建空白蒙版    mask = np.zeros(image_size[::-1], dtype=np.uint8)  # 注意：OpenCV使用(height, width)
    ...
    # 方法1：使用凸包来创建实心蒙版（适用于形状相对简单的结节）
    try:
        from scipy.spatial import ConvexHull
        points_array = np.array(unique_points)
        hull = ConvexHull(points_array)
        hull_points = points_array[hull.vertices].astype(np.int32)
        cv2.fillPoly(mask, [hull_points], 255)
        
        # 检查是否需要额外处理（如果原始点与凸包差异很大）
        original_area = cv2.contourArea(np.array(unique_points, dtype=np.int32))
        hull_area = cv2.contourArea(hull_points)
        
        # 如果凸包面积与原始轮廓面积差异较大，使用原始点填充
        if abs(original_area - hull_area) / max(original_area, 1) > 0.3:
            raise Exception("使用原始点填充")
            
    except:
        # 方法2：直接使用原始点填充，并添加形态学操作来填充空洞
        polygon_points = np.array(unique_points, dtype=np.int32)
        ...
        for point in [(0, 0), (0, w-1), (h-1, 0), (h-1, w-1)]:
            if mask_copy[point] == 0:
                cv2.floodFill(mask_copy, flood_mask, point, 128)
        
        # 将未被填充的区域（即被边界包围的区域）设为前景
        mask[mask_copy == 0] = 255
...
if __name__ == "__main__":
    # 设置Unet输出文件夹
    unet_train_folder = r"D:\MyFile\qq_3045834499\Unet"    # 全局索引计数器，确保所有文件名唯一
    global_index = 0
